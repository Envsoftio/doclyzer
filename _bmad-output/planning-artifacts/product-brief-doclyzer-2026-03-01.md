...
- **Primary model – credits (recharge):** User has a **credit balance**. Credits are **deducted per report** when a report is uploaded and fully analysed (parse + store + AI summary/insights). Optional: extra deduction for heavy AI (e.g. chat with report, lifestyle suggestions) or per active share link—product decision. User **recharges** by buying **credit packs** (e.g. 5, 15, 50 credits); larger packs at better per-credit price. One-time RevenueCat purchase via Google Pay/platform billing where available; no subscription required.
...
- **Summary:** Credits-first; recharge via credit packs (one-time) **and** subscription (monthly credits + features)—both in v1. Payments are handled through RevenueCat, using Google Pay/platform billing where available.

### Monetization plans (suggested tiers)

- **Payments:** Processed via **RevenueCat**, using Google Pay/platform billing where available for credit packs and subscriptions. Promo codes are applied in our backend before starting a RevenueCat purchase; see “Promo codes with RevenueCat” below.
- **Identifying user country / pricing region (show prices in local currency):** We need a **pricing region** (e.g. India, US, EU) per user or per session to choose currency (INR / USD / EUR) and the correct price from the plan table.
  ...
  - **Override – Explicit choice:** Let the user set **Country / Region** in account or checkout (e.g. “Prices in: India (₹) | United States ($) | Europe (€)”). Store per account so pricing region and currency stay consistent with the RevenueCat offering/product selected for checkout. Useful for expats, travel, or when IP/locale are wrong.
...
- **Promo codes with RevenueCat:** Promo logic is owned by our backend; the backend validates and records the promo before mobile starts the RevenueCat purchase flow.
  - **Validation:** Before starting any RevenueCat purchase, backend validates the promo (code exists, not expired, usage cap not exceeded, applicable to selected product—e.g. credit pack or subscription). Invalid/expired code returns a clear error; no purchase flow is started until valid.
  - **Credit pack (one-time):** Backend computes discounted amount from pack price and promo, records a pending discounted order, and mobile starts the matching RevenueCat product/offering through Google Pay/platform billing where available. On trusted RevenueCat webhook reconciliation, record redemption, credit the user’s balance with the pack’s credits, and log (code, user, pack, discount, RevenueCat purchase/event id).
  - **Subscription (v1):** Use RevenueCat offerings/products for subscription purchase and entitlement state; redemption is logged after trusted RevenueCat webhook reconciliation for superadmin analytics.
  - **Idempotency:** Idempotency keys or idempotent redemption records so retries don’t double-apply or double-count.
  - **Superadmin analytics:** Redemption records (our DB) = source for “who used which code, when, revenue impact”; RevenueCat webhook events are the source for payment reconciliation.
...
- **System observatory (v1+):** The dashboard will eventually surface every operational and business telemetry slice in one scrollable view: total accounts and active users, login/verification attempts, upload/report activity, share-link issuance, payment/revenue summaries (credit packs, subscriptions, refunds, invoices), file/report inventories (queued, processing, succeeded, failed), governance alerts (suspicious activity scores, review queues, audit spikes), and protective-restriction state. Each widget supports date, geography, or product filters and links to the underlying drill-down (e.g. account/organization detail, report list, payment event). Output stays PHI-safe by showing account-level identifiers only and sanitizing exports (CSV/JSON), while every query enforces `X-Admin-Action-Token` validation and emits an audit entry with actor/action/target/outcome.
...
- **Credits & recharge (primary monetization):** User has a credit balance (account-level). Each report analysed consumes credits (e.g. 1 credit per report; rules configurable in plan table). Free tier: small initial credits (e.g. 3–5) or a one-time report cap (e.g. 5 total). User **recharges** by buying **credit packs** (e.g. 5, 15, 50 credits) through RevenueCat with Google Pay/platform billing where available; balance is credited only after trusted RevenueCat webhook reconciliation. Subscription (v1): monthly credit allowance + features (see “Monetization plans” in this brief). Both credit packs and subscription available from day one.
