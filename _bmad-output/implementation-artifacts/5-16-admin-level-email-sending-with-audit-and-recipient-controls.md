...
- 2026-04-03: Code review fix — normalized approval token error message: missing EMAIL_ADMIN_APPROVAL_SECRET now returns the same "Invalid approval token" message as a wrong token, preventing callers from detecting misconfigured state. Added TODO comment on recordAudit flagging that email admin audit events should migrate to a dedicated entity (currently stored in SuperadminAuthAuditEventEntity).
